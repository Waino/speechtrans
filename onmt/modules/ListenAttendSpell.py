...
# apply same module on each timestep of every instance
...
class pBLSTMLayer(nn.Module):
    def __init__(self, input_feature_dim, hidden_dim,
                 rnn_unit='LSTM', dropout_rate=0.0):
...
class Listener(nn.Module):
    def __init__(self, input_feature_dim, listener_hidden_dim, rnn_unit, use_gpu, dropout_rate=0.0, **kwargs):
        ...
        # Listener RNN layer
        self.pLSTM_layer1 = pBLSTMLayer(
            input_feature_dim, listener_hidden_dim, rnn_unit=rnn_unit, dropout_rate=dropout_rate)
        self.pLSTM_layer2 = pBLSTMLayer(
            listener_hidden_dim*2, listener_hidden_dim, rnn_unit=rnn_unit, dropout_rate=dropout_rate)
        self.pLSTM_layer3 = pBLSTMLayer(
            listener_hidden_dim*2, listener_hidden_dim, rnn_unit=rnn_unit, dropout_rate=dropout_rate)
...
class Speller(nn.Module):
    def __init__(self, output_class_dim,  speller_hidden_dim, rnn_unit, speller_rnn_layer, use_gpu, max_label_len,
        ...
        self.rnn_layer = self.rnn_unit(output_class_dim + speller_hidden_dim,
                                       speller_hidden_dim,
                                       num_layers=speller_rnn_layer)
        self.attention = Attention(mlp_preprocess_input=use_mlp_in_attention,
...
class Attention(nn.Module):
    def __init__(self, mlp_preprocess_input, preprocess_mlp_dim, activate,
                 mode='dot', input_feature_dim=512):
    ...
    def forward(self, decoder_state, listener_feature):
        if self.mlp_preprocess_input:
            ...
            comp_listener_feature = self.relu(
                TimeDistributed(self.psi, listener_feature))
        else:
            comp_decoder_state = decoder_state
            comp_listener_feature = listener_feature

        if self.mode == 'dot':
            energy = torch.bmm(comp_decoder_state,
                               comp_listener_feature.transpose(1, 2)).squeeze(dim=1)
        else:
            # TODO: other attention implementations
            assert False
        attention_score = self.softmax(energy)
        context = torch.sum(listener_feature * 
            attention_score.unsqueeze(2).repeat(1, 1, listener_feature.size(2)), dim=1)
